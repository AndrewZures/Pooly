defmodule Pooly.PoolServer do
  ...
  defmodule State do
    defstruct pool_sup: nil, size: nil, mfa: nil, worker_sup: nil, workers: nil, name: nil, monitors: nil, max_overflow: nil, overflow: nil, waiting: nil
  end
  ...
  def handle_info(:start_worker_supervisor, state = %{pool_sup: pool_sup, mfa: mfa, size: size, name: name}) do
    {:ok, worker_sup} = Supervisor.start_child(pool_sup, supervisor_spec(name, mfa)) # start worker sup
    workers = prepopulate(size, worker_sup) # populate workers
  ...
  def handle_call({:checkout, block}, {from_pid, _ref} = from, state) do
    %{ worker_sup: worker_sup, workers: workers, monitors: monitors, overflow: overflow, max_overflow: max_overflow, waiting: waiting } = state
    case workers do
      [worker|rest] ->
  ...
  def handle_call(:status, _from, %{workers: workers, monitors: monitors} = state) do
    {:reply, {state_name(state), length(workers), state, :ets.info(monitors, :size)}, state}
  end
  ...
  defp supervisor_spec(name, mfa) do
    opts = [id: name <> "WorkerSupervisor", restart: :permanent]
